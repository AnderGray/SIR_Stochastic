function [samples_fT_D, log_fD] = tmcmc(log_fD_T, fT, sample_from_fT, N, Npar)
...
plot_graphics    = false;
burnin           = 10;
lastburnin       = 10;  % burnin in the last iteration
...
thetaj1   = zeros(N, D);
log_fD_T_thetaj = zeros(size(thetaj,1),1);
%% Main loop
while pj < 1
    ...
    Nbatches = Npar;
    NperBatch = N/Nbatches;         % Must be an integer, and partition N exactly
    thetajIn = reshape(thetaj,NperBatch,Nbatches, size(thetaj,2));
    log_fD_T_thetajIn = zeros(NperBatch, Nbatches);
    
    fprintf('Evaluating the likelihood in %d batches.\n', Nbatches);
    
    parfor jk = 1:Nbatches
        for kk = 1:NperBatch
           log_fD_T_thetajIn(kk,jk) = log_fD_T(reshape(thetajIn(kk,jk,:),1,size(thetaj,2)));
        end
    end
    log_fD_T_thetaj = reshape(log_fD_T_thetajIn, size(thetaj,1),1);
    
    % Fro non-paralell
    %log_fD_T_thetaj1 = zeros(N,1);
    %for jk = 1:N
    %    log_fD_T_thetaj1(jk) = log_fD_T(thetaj(jk,:));
    %end
